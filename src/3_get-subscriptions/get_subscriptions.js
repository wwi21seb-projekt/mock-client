...
function searchSubscriptions() {
    ...
    const url = GlobalSettings.apiUrl + '/subscriptions/' + username +'?type=' + SubscriptionType +  '&offset=' + offset + '&limit=' + limit;
...
function displayResults(data, statusCode) {
    ...
    if (data.records && data.records.length > 0) {
        ...
        data.records.forEach(subscription => {
            const subscriptionDiv = document.createElement('div');
            subscriptionDiv.innerHTML = `FollowerId: ${subscription.followerId}, FollowingId: ${subscription.followingId}, Username: ${subscription.username}`;
            responseDiv.appendChild(subscriptionDiv);
        });
    } else {
        responseDiv.innerHTML = 'No Subscriptions found found';
    }
